...
- 插件市场：自建仓库索引，支持 SOCKS5/HTTP 代理；**安装走 NDJSON 进度流**（下载百分比 + 停止/解压/启动阶段，可取消）
- 出站标识按插件配置（UA / 客户端名称 / 版本 / CLI 版本），默认对齐官方分发包
- **对话 UA 三级解析**：路由级 UA > 全局网关 UA > 客户端自带（只解析下发，插件按需透传）
- **客户端指纹注入**：核心按入口协议生成 Claude Code / Codex 指纹头（user-agent / x-app / anthropic-beta / originator / session_id / turn 元数据）下发到插件 extra，插件按需采用
- **系统信息 / 备份 / 恢复**：运行时信息与各表计数；备份为 VACUUM INTO 一致性快照 + secret.key 打包 zip；恢复走暂存目录，重启时换入（旧库自动另存 .bak-时间戳）
...
- [x] 设置（网关 / 日志 / 网络 / **按插件的出站标识** / 会话粘性 / **系统与备份** / 管理员密码）
- [x] 初始化引导（首次启动创建管理员）
- [x] 路由级 UA 与全局网关 UA（应用优先级：路由 > 全局 > 客户端）
- [x] 客户端指纹注入（Claude Code / Codex，按入口协议下发）
- [x] 插件安装进度流 + 系统信息 / 备份 / 恢复

后续可做：插件契约 v2（多实例，一个插件挂多个站点）、市场多源、删除影响面预览。
