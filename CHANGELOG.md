...
**前端**
- Fixed 账号页的插件筛选与分组块顺序每次刷新都可能变：根因是后端插件列表来自 map 遍历，
  现在 `/admin/plugins` 按品牌名稳定排序，前端再按品牌名兜底排序（下拉、设置页同样受益）。
- Added 账号「编辑」弹窗的出站代理：分组已绑的代理用虚线标出并标注「继承分组」，
  下方补一句当前生效说明（账号级 > 分组级 > 直连），不再让人以为"没绑定"。
- Added 账号表格「分组」列与账号详情新增「出站代理」，显示当前实际生效的代理。

**后端**
- Changed `GET /admin/groups` 每行新增 `proxy_ids`；`GET /admin/accounts` 每行新增 `proxy_ids`
  （账号级绑定；空数组 = 继承分组代理 / 直连）。
- Fixed `plugin.Manager.Names()` 返回排序后的插件名（map 遍历顺序随机 ⇒ 顺序乱跳的根因）。

## v1.3.0 — 模型中心标注来源渠道（2026-09-23）

**模型中心**
- Added 「来源渠道」列：每个模型列出提供它的插件；同一插件多个账号时显示 ×N。
- Added 同名模型被多个插件提供时打「多源」标记，悬停列出各渠道账号数——
  不同渠道的同名模型行为/计费可能不同，路由也是按账号分组解析，必须让用户看得见。
- Added 顶部「来源渠道」筛选（全部 / 各插件，带模型数）；搜索框也能按渠道名匹配。
- Added 概览卡片新增「多渠道同名模型」计数。

**后端**
- Changed `GET /admin/models` 每行新增 `plugins[]` 与 `sources[{plugin,accounts}]`
  （保留旧 `plugin` 字段兼容旧前端）：同名模型按插件聚合账号数，来源名稳定排序。
...
**前端**
- Fixed 账号详情「在线测试」的模型输入从 `input+datalist`（很多浏览器不弹列表、只能手打）
  改成真正的下拉选择，选项来自该账号的模型目录（显示上游展示名 + 内部 key）。
- Added 「同步模型」按钮：随时拉一次上游模型目录；模型目录为空时打开抽屉自动同步一次。
- Added 下拉里保留「自定义模型名…」入口，仍可手输未列出的模型。
